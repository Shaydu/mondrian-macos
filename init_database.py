...
**CRITICAL**: If reference images are provided in the prompt, you MUST include a case_studies array with up to 3 entries. Each case study must cite the EXACT image title from the references and explain how it demonstrates mastery in a specific dimension."""
    ...
    # Populate disclaimer if empty
    cursor.execute('SELECT COUNT(*) FROM config WHERE key = ?', ('disclaimer',))
    if cursor.fetchone()[0] == 0:
        disclaimer = """AI-Generated Analysis: This report is generated by artificial intelligence and represents a simulated perspective of the named advisor. It is not a professional critique from an actual photographer or art expert.
