#!/usr/bin/env python3
"""
Direct LoRA Strategy Test with Enhanced Debugging
Tests the LoRA strategy in isolation to debug model output, including raw response capture
"""
import sys
import os
import json
...
# IMPORTANT: We need to patch the parse_json_response function to capture the raw output
# Let's do this BEFORE importing lora strategy

def capture_model_output():
    """Monkey-patch to capture raw model output for debugging"""
    
    # Import the module that will be patched
    import ai_advisor_service
    
    original_parse = ai_advisor_service.parse_json_response
    captured_output = {"raw": None}
    
    def patched_parse(response):
        """Wrapper that captures the raw response"""
        print(f"\n[DEBUG] === RAW MODEL OUTPUT ===")
        print(f"[DEBUG] Response length: {len(response)} characters")
        print(f"[DEBUG] Response type: {type(response)}")
        print(f"\n[DEBUG] First 1500 chars:")
        print(response[:1500])
        ...
        captured_output["raw"] = response
        
        # Call original parser and handle errors
        try:
            result = original_parse(response)
            print(f"[DEBUG] ✓ Parsing successful, returned dict with keys: {list(result.keys()) if isinstance(result, dict) else 'N/A'}")
            return result
        except Exception as e:
            print(f"[DEBUG] ✗ Parsing failed: {e}")
            raise
    
    ai_advisor_service.parse_json_response = patched_parse
    return captured_output

# Capture output before importing strategies
captured = capture_model_output()
...
def test_lora_strategy():
    ...
    print("\n" + "="*70)
    print("LORA STRATEGY DIRECT TEST - ENHANCED DEBUG")
    print("="*70 + "\n")
    ...
    try:
        ...
        # Print captured raw output
        if captured["raw"]:
            print(f"\n" + "="*70)
            print("CAPTURED RAW MODEL OUTPUT (for reference)")
            print("="*70)
            print(captured["raw"][:2000])
            if len(captured["raw"]) > 2000:
                print(f"\n... [{len(captured["raw"]) - 2000} more characters] ...\n")
                print(captured["raw"][-500:])
        ...
        if captured["raw"]:
            print(f"\n" + "="*70)
            print("RAW MODEL OUTPUT (WHAT THE MODEL ACTUALLY GENERATED)")
            print("="*70)
            print(f"\nLength: {len(captured['raw'])} characters\n")
            print(captured["raw"][:3000])
            if len(captured["raw"]) > 3000:
                print(f"\n... [{len(captured["raw"]) - 3000} more characters] ...\n")
                print(captured["raw"][-1000:])
