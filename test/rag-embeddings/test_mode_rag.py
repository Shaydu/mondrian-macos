#!/usr/bin/env python3
"""
RAG Mode Test - Minimal, Standalone Test

Tests the base model with RAG enabled.
RAG loads reference images and embeddings for context, using more memory than baseline.

Usage:
    python3 test_mode_rag.py
    python3 test_mode_rag.py --verbose
"""
...
def check_prerequisites():
    ...
    try:
        ...
        print_info("Make sure service is running: ./mondrian.sh --restart --mode=rag")
        return False
...
def run_rag_test(verbose=False):
    """Run single RAG analysis."""
    print_header("Running RAG Analysis Test")
    
    print_info(f"Mode: {MODE}")
    print_info(f"Advisor: {ADVISOR}")
    print_info(f"Timeout: {TIMEOUT}s")
    print_info(f"Sending analysis request with RAG enabled...")
...
def main():
    parser = argparse.ArgumentParser(
        description="RAG Mode Test - Tests base model with RAG enabled"
    )
    ...
    # Run test
    if run_rag_test(verbose=args.verbose):
        print_header("TEST PASSED ✓")
        print(f"{GREEN}RAG mode is working correctly!{NC}\n")
        return 0
    else:
        print_header("TEST FAILED ✗")
        print(f"{RED}RAG mode failed. Check logs/ai_advisor_service_*.log{NC}\n")
        return 1
